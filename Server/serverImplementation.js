...
module.exports = {
    getByUserId: function (call, callback) {
        ...
        for (let key in md) { console.log(key, md[key] )};
    ...
    getAll: function (call) {
        data.users.forEach( usr => call.write({ user: usr }));
        call.end();
    },
    ...
    addImage: function (call, callback) {
        ...
        call.on('end', function () {
            ...
            console.log(`File_Size: ${result.length} bytes`);
        });
    },
    saveAll: function (call) {
        ...
        call.on('end', function () {
            data.users.forEach( usr => console.log(usr));
            call.end();
        });
    }
}